...
async function redirectToAccount(driver, targetAccount) {
  // Yeni sekme aç
  await driver.executeScript(`window.open('https://x.com/${targetAccount.twitterAccount}', '_blank');`);
  ...
  // Yeni sekme üzerinde işlem yap
  await driver.switchTo().window(newWindowHandle);

  // Hedef hesap sayfası yüklenene kadar bekleyin
  await driver.wait(until.urlContains(targetAccount.twitterAccount), 10000);

  // 5 saniye daha bekleyin (opsiyonel)
            ...
            try {
              ...
              try {
                ...
                if (!tweets.some(tweet => tweet.tweetText === tweetText) && !isToday) {
                  ...
                  const fileName = `${coinName}_${targetAccount.twitterAccount}_tweets.csv`;
